...
public class Carpenter {
    ...
    double input_alpha, input_beta, input_A1, input_B1, input_g_mu, input_SB, result_S1;

    double result_gamma, result_Lp, result_Lc,
            result_H1, result_H2, result_M2, result_M1,
            result_N2, result_N1, result_K2, result_K1,
            result_Area, result_Nr, result_Pk, result_LBc, result_MB1;

    double result_LAc, result_LBp, result_SA,
            result_HK, result_HB1, result_NB2,
            result_NA1, result_MA1, result_PP, result_NB1, result_KB2,
            result_SB1, result_LAp, result_MA2, result_NA2, result_KA2,
            result_SA1, result_HA1, result_HA2, result_MB2;
    ...
    void countRoof1Values() {
        ...
        result_Area = result_Lp * input_B / 10000;
    }
    ...
    void countRoof2Values() {
        ...
        result_S1 = Math.abs(result_K2 - input_S / Math.sin(Math.toRadians(input_theta)));
        ...
        result_Pk = result_Cd / no_areas - input_g_kr;
        result_Area = result_Lp * input_B * 2 / 10000;
    }
    ...
    void countRoof3Values() {
        ...
        result_LBc = result_LBp + input_s_kr * Math.tan(Math.toRadians(input_beta));
        ...
        result_MB2 = result_LBp - input_E / Math.cos(Math.toRadians(input_beta)) + result_WB;
        result_MB1 = result_MB2 - input_SB / Math.sin(Math.toRadians(input_beta));
        result_NB2 = result_LBp - (input_B + input_E - input_B1) / Math.cos(Math.toRadians(input_beta)) + result_WB;
        result_NB1 = result_NB2 - input_SB / Math.sin(Math.toRadians(input_beta));
        result_KB2 = input_g_pk / 2 / Math.cos(Math.toRadians(input_beta));
        result_SB1 = input_SB / Math.sin(Math.toRadians(input_beta));

        result_LAp = (input_A + input_D) / Math.cos(Math.toRadians(input_alpha));
        result_LAc = result_LAp + input_s_kr * Math.tan(Math.toRadians(input_alpha));
        result_MA2 = result_LAp - input_D / Math.cos(Math.toRadians(input_alpha)) - result_WA;
        result_MA1 = result_MA2 - input_SB / Math.sin(Math.toRadians(input_alpha));
        result_NA2 = result_LAp - (input_A + input_D - input_A1) / Math.cos(Math.toRadians(input_alpha)) - result_WA;
        result_NA1 = result_NA2 - input_SB / Math.sin(Math.toRadians(input_alpha));
        result_KA2 = input_g_pk / 2 / Math.cos(Math.toRadians(input_alpha));
        ...
        result_SA1 = result_SA / Math.sin(Math.toRadians(input_alpha));
        ...
        result_HA1 = result_HK - result_SA + input_SB - (input_A1 - result_PP - input_g_pk / 2) * Math.tan(Math.toRadians(input_alpha));
        result_HA2 = result_HA1 - (input_A - input_A1) * Math.tan(Math.toRadians(input_alpha));
    ...
    public HashMap<String, Double> prepareResults(int roof_type) {
        ...
        if (roof_type == 1) {
            ...
            results.put("SB", input_SB);
            results.put("HA1", result_HA1);
            results.put("HA2", result_HA2);
            results.put("SA1", result_SA1);
            results.put("LAp", result_LAp);
            results.put("KA2", result_KA2);
            results.put("NA2", result_NA2);
            results.put("MA2", result_MA2);
            results.put("SB1", result_SB1);
            results.put("LBc", result_LBc);
            results.put("KB2", result_KB2);
            results.put("NB1", result_NB1);
            results.put("MB1", result_MB1);
            results.put("MB2", result_MB2);
